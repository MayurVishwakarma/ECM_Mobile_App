                    ...
                    child: FittedBox(
                        child: Text(
                            ...
                            style: getLastRoutineStatusStyle(
                                _DisplayList[index].routineStatus!,
                                _DisplayList[index].nextScheduleDate!)
                            /*_DisplayList[index].routineStatus == 2
                          ? TextStyle(
                              fontSize: 8,
  ...
  /*Future<void> _scount() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<RoutineScountModel> fetchedData =
            (json['data']['Response'] as List)
                .map((e) => RoutineScountModel.fromJson(e))
                .toList();

        if (fetchedData.isNotEmpty) {
          setState(() {
            scountList.addAll(fetchedData);
          });
        }
      }
    } catch (err) {
...
*/
  ...
  Future<void> fetchRoutineTime() async {
    ...
    // return null; // Return null if the API call fails or the "Days" value is not available
  }
  ...
  /*Future<void> getRoutineDay() async {
    try {
      SharedPreferences preferences = await SharedPreferences.getInstance();
      String? conString = preferences.getString('ConString');
      ...
      if (response.statusCode == 200) {
        var json = jsonDecode(response.body);
        List<RoutineTimeModel> Result = <RoutineTimeModel>[];
        json.forEach((v) => Result.add(new RoutineTimeModel.fromJson(v)));
        setState(() {
          scount = Result.first.days;
        });
      } else {
        throw Exception('Failed to load API');
      }
...
*/
  /*Future<void> routinetime() async {
    try {
      ...
      if (response.statusCode == 200) {
        var json = jsonDecode(response.body);
        List<RoutineTimeModel> fetchedData = (json['data']['Response'] as List)
            .map((e) => RoutineTimeModel.fromJson(e))
            .toList();

        if (fetchedData.isNotEmpty) {
          setState(() {
            routineTime = fetchedData.first.days;
          });
        }
      }
    } catch (err) {
